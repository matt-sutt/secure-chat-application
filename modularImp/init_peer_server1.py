...
#logging
setup_logging()  
logger = logging.getLogger("InitPeerServer1") 
...
async def _start_and_join(server: Server, introducer_host: str, introducer_port: int):  # start and join
    ...
    async def _join():  # join introducer
        ...
        ok = await server.send_server_hello_join(introducer_host, introducer_port, introducer_id="*")  # join hello
        if not ok:  # join failed
